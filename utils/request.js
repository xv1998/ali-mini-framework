...
const request = function (options) {
    ...
    // 校验登录态
    function _validateLoginState() {
        // 检查skey
        let loginState = cache.get(cacheKeyMap.loginState)
        if (!loginState) {
            return Promise.reject(new Error('登录秘钥不存在，需要重新登录'))
        }
        ...
        if (!skey || new Date(skeyExpiresAt) < new Date()) {
            if (!refresh_key || new Date(refreshKeyExpiresAt) < new Date()) {
                ...
                return Promise.reject(new Error('refresh_key无效，需要重新登录'))
            } else {
                // 刷新skey，重新请求，并更新登录态
                return _refreshLoginState(refresh_key)
            }
        }
        return Promise.resolve()
    }

    function _refreshLoginState(refresh_key) {
        ...
        return request({
            ...
            if (res.data.code !== '0') {
                logout()
                return Promise.reject(new ErrorMessage('刷新skey失败', res.data.code, { needReport: false }))
            }
    ...
    function _req(_options) {
        ...
        return wxRequest(_options)
            .catch(err => {
                my.hideLoading()
                return Promise.reject(new ErrorMessage(`发送请求出错 ${err.errMsg}`))
            })
            .then(res => {
                // 检验登录态是否有效
                if (_options.needLogin && (
                    ...
                    if (!hasRetry) {
                        hasRetry = true
                        // 登录态失效，刷新后重试
                        return _refreshLoginState(cache.get(cacheKeyMap.loginState).refresh_key)
                            .then(() => {
                                beginTime = Date.now()
                        ...
                        logout()
                        my.hideLoading()
                        return Promise.reject(new ErrorMessage('登录态失效，刷新后重试仍失败'))
                    }
                }
    ...
    // 请求附带的额外信息
    _options.header['from'] = 'mini'
    _options.header['app_id'] = config.app_id
    _options.header['release'] = config.release
    let currentPage = getCurrentPages()
    _options.header['page'] = currentPage.length > 0 && currentPage[currentPage.length - 1] ? currentPage[currentPage.length - 1].route : 'no page'

    // socket
    _options.header['socket_id'] = app.globalData && app.globalData.socket_id
    ...
    let beforeRequestPromise
    if (_options.needLogin) {
        beforeRequestPromise = _validateLoginState().catch(err => {
            // 登录态过期，注销
            logout()
        ...
        beforeRequestPromise = Promise.resolve()
    }
...
//         if (res.data.code !== '0') {
//             await logout()
//             throw new ErrorMessage('刷新skey失败', res.data.code)
//         }
//         // 更新登录秘钥
//         let loginState = {
//             skey: res.data.skey,
...
//         } catch (e) {
//             wx.hideLoading()
//             throw new ErrorMessage(`发送请求出错 ${e.message}`)
//         }
//
//         // 检验登录态是否有效
//         if (_options.needLogin && (
...
//                 await logout()
//                 wx.hideLoading()
//                 throw new ErrorMessage('登录态失效，刷新后重试仍失败')
//             }
//         }
//         console.log(`url: ${_options.url} time: ${Date.now() - beginTime}ms`, res)
//         wx.hideLoading()
...
//  */
// const getWxLoginResult = function () {
//     return wxLogin()
//         .then(loginResult => {
//             return wxGetUserInfo().then(userResult => {
//                 return {
...
//                 default:
//                     throw new ErrorMessage('Oauth登录失败', res.data.code)
//             }
//             let oauthSessionKey
...
//             throw new ErrorMessage('Oauth登录过期，请重新登录')
//         }
//         if (res.data.code !== '0') {
//             throw new ErrorMessage('Oauth认证错误', res.data.code)
//         }
//         [syllabusSessionKey, syllabusSessionValue] = res.header['set-cookie'].split(';')[0].split('=')
//         if (!syllabusSessionKey || !syllabusSessionValue) {
//             throw new ErrorMessage('获取syllabusSession失败')
//         }
//         // 存储业务后台session
...
//
//         if (data.code !== '0') {
//             throw new ErrorMessage('小程序微信登录出现错误', data.code)
//         }
//         let loginState = {
//             skey: data.skey,
...
//         throw new ErrorMessage('no oauthSession')
//     }
//     let res
